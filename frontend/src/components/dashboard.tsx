...
const scenarios = [
  ...
  {
    id: 'desktop',
    title: 'Scenario Desktop',
    description: 'A short description of the scenario',
  },
]

export function Dashboard() {
  const [selectedScenarios, setSelectedScenarios] = useState<string[]>(['mobile', 'desktop'])

  const getScenarioColor = (scenarioId: string) => {
    return chartConfig[scenarioId as Scenario].color
  }
        ...
        <div className="flex mt-8 gap-4">
          {scenarios.map(scenario => (
            <ToggleGroup.Item
              ...
              key={scenario.id}
              value={scenario.id}
              style={selectedScenarios.includes(scenario.id) ? { '--tw-shadow-color': getScenarioColor(scenario.id) } as React.CSSProperties : undefined}
            >
              <ScenarioCard
                className="w-40"
