...
export function DistrictDetail({ districtData, districtName, selectedYear, onYearChange }: DistrictDetailProps) {
  ...
  useEffect(() => {
    ...
    const years = [...new Set(districtData.map(item => item.Year))].sort((a, b) => a - b)
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      <p className="text-muted-foreground">
        This section shows an informative visual breakdown of land cover and environmental metrics over time,
        ...
        ? (
            <Card>
              <CardContent className="pt-6">
                <p>No data available for this district. Please select another district.</p>
              </CardContent>
            </Card>
          )
        : (
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={filteredData} margin={{ top: 20, right: 20, left: 0, bottom: 20 }}>
                      ...
                      <Legend />
                      <Bar dataKey="openShrublands" fill="#8884d8" name="Open Shrublands (%)" />
                      <Bar dataKey="grasslands" fill="#82ca9d" name="Grasslands (%)" />
                      <Bar dataKey="croplands" fill="#ff8042" name="Croplands (%)" />
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={filteredData} margin={{ top: 20, right: 20, left: 0, bottom: 20 }}>
                      ...
                      <Tooltip formatter={value => typeof value === 'number' ? value.toFixed(2) : value} />
                      <Legend />
                      <Line type="monotone" dataKey="precip" stroke="#8884d8" name="Precipitation (mm)" />
                      <Line type="monotone" dataKey="gpp" stroke="#82ca9d" name="GPP (kg_C/m²/year)" />
                      <Line type="monotone" dataKey="popDensity" stroke="#ff7300" name="Population Density (People/km²)" />
                    </LineChart>
                  </ResponsiveContainer>
                </CardContent>
              </Card>
                      ...
                      <TableBody>
                        {filteredRawData.map((row, index) => (
                          <TableRow key={index}>
                            ...
                            <TableCell>{row.LandCoverLabel || `Class ${row.LandCoverClass}`}</TableCell>
                            <TableCell>
                              {row.Percentage.toFixed(2)}
                              %
                            </TableCell>
                            <TableCell>{row['Precipitation (mm)']?.toFixed(2) || 'N/A'}</TableCell>
                            <TableCell>{row['GPP (kg_C/m²/year)']?.toFixed(2) || 'N/A'}</TableCell>
                            <TableCell>{row['Population Density (People/km²)']?.toFixed(2) || 'N/A'}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
